...
export default function ProjectRoom() {
  ...
  return (
    <main ref={root} className="bg-base">
      ...
      {/* RoomEntrance — 배경 이미지 + 톤 오버레이 + 오브젝트팩 + 콘텐츠 */}
      <section
      ...
      {/* OriginStory — 웹툰식 세로 내러티브: 원점(Adwise)→검증→전환→확장(PULSE) */}
      {room.originStory && (
          ...
          <dl className="mt-8 flex flex-col">
            {(
              [
                ...
                ['성과/검증', room.infoDock.verification],
                ['UX Writing Point', room.infoDock.uxWritingPoint],
                ['배운 점', room.infoDock.learning],
              ] as const
            ).map(([k, v]) => (
              <Reveal key={k}>
