...
export const ROOMS: Record<ProjectKey, RoomData> = {
  pulse: {
    ...
    objectPack: '/images/design-assets/object-packs/pulse_objectpack.png',
    entranceLine:
    ...
    mainWall: {
      ...
      caption: '데이터를 보여주는 데서 멈추지 않고, 다음 홍보 행동으로 연결하는 대시보드',
    },
    evidence: [],
    evidenceGroups: [
      {
        ...
        items: [
          {
            ...
            caption: '검증 근거 — 리뷰 기반 페르소나 연구로 UX 소논문 최우수상 수상',
          },
        ],
      },
    ],
    ...
    infoDock: {
      problem:
        '외식업 사장님은 리뷰, 고객 특성, 홍보 콘텐츠, 가게 현황을 따로 관리해야 한다. 데이터는 흩어져 있고, 무엇을 보고 어떤 홍보 행동을 해야 하는지 결정하기 어렵다.',
      insight:
        '문제는 정보 부족이 아니라 정보를 보고 다음 행동으로 이어지는 흐름의 부재였다. 그래서 분석 결과보다 “오늘 무엇을 할지”를 우선순위로 올렸다.',
      role:
      ...
      uxWritingPoint:
        '사장님이 이해하기 쉬운 지표명, 오늘의 기회 신호, 추천 이유 문장을 중심으로 AI 결과를 “설명”이 아니라 “행동 제안”으로 읽히게 설계했습니다.',
      learning:
        '데이터 분석은 결과 그 자체가 아니라, 사용자의 다음 행동으로 번역될 때 비로소 가치가 생겼습니다.',
    },
  },
  pickfit: {
    key: 'pickfit',
    ...
    objectPack: '/images/design-assets/object-packs/pickfit_objectpack.png',
    entranceLine:
    ...
    casePanels: [
      {
        ...
        body:
          '처음부터 패션 쇼핑몰을 더 많이 탐색하게 만드는 것이 아니라, 상황과 취향을 말하면 사용자가 바로 결정할 수 있는 후보를 좁혀주는 제품으로 기획했다.',
      },
      {
        eyebrow: 'design system',
        title: '브랜드부터 UI 상태까지 독립 디자인 시스템으로 설계',
        body:
          '라임과 블루를 핵심 인상으로 잡고, 로고부터 컬러·타이포·컴포넌트, 로딩·결과·아이템 확인 상태까지 하나의 서비스 언어로 맞췄다.',
      },
      {
        eyebrow: 'ai workflow',
        title: 'AI를 초안 생성기가 아니라 설계·검증 흐름으로 사용',
        body:
          'Skills는 반복되는 작업 기준을 고정할 때, subagent는 리서치·UX 리뷰·구현 점검을 나눌 때, 하네스 엔지니어링은 추천 입력/출력과 실패 케이스를 검증할 때 사용했다.',
      },
    ],
    aiUsage: {
      intro:
      ...
      blocks: [
        {
          ...
          how: [
            '사용자 입력을 자유 문장이 아니라 상황·예산·취향·핏·색상 같은 구조화된 조건으로 나눴다.',
            '프롬프트에는 역할, 맥락, 제약, 출력 형식을 함께 넣어 추천 결과가 UI에 바로 들어올 수 있게 설계했다.',
            '추천 결과는 코디명, 추천 이유, 가격대, 구성 아이템처럼 사용자가 검토할 수 있는 단위로 받도록 구성했다.',
          ],
        ...
        },
        {
          ...
          how: [
            '매번 새로 판단하지 않도록 PickFit의 기준을 “선택 피로를 줄이는 위임형 추천 UX”로 고정했다.',
            '기획 문장, 버튼/라벨 문구, 화면 우선순위, 포트폴리오 설명이 같은 관점을 유지하는지 점검했다.',
            '라임·블루 컬러, 질문형 UX Writing, 추천 이유 중심 화면 같은 규칙을 반복 적용했다.',
          ],
        ...
        },
        {
          ...
          how: [
            '리서치 역할은 쇼핑 UX가 직접 탐색에서 AI 위임으로 이동한다는 문제의식을 정리하는 데 사용했다.',
            'PM 역할은 기능을 늘리는 대신 추천 흐름의 범위와 우선순위를 좁히는 데 사용했다.',
            'UX 리뷰 역할은 질문형 입력, 로딩 상태, 추천 이유, 구성 아이템 화면이 사용자의 결정 확신을 돕는지 점검했다.',
            'FE/QA 역할은 화면 크기, 반응형, 컴포넌트 구조, 빌드 오류처럼 구현 품질을 확인하는 데 사용했다.',
          ],
          output: '기획 인사이트, UI Flow, 디자인 시스템, 포트폴리오 상세 레이아웃을 역할별로 검토',
        },
        {
          eyebrow: 'harness engineering',
          title: 'AI 추천 결과를 감이 아니라 기준으로 검증',
          usedWhen: '추천 기능이 “그럴듯한 문장”에 머물지 않고 실제 제품 화면에 들어갈 수 있는지 확인하는 단계',
          how: [
            '대표 입력 예시를 만들었다. 예: 출근룩, 10~20만 원, 캐주얼, 레귤러 핏, 특정 색상 선호.',
            '기대 출력 기준을 정했다. 예: 예산 안에 들어오는지, 상황과 맞는지, 추천 이유가 구체적인지, 구성 아이템이 분리되는지.',
            '실패 케이스를 점검했다. 예: 너무 일반적인 추천 이유, 예산 초과, 조건과 맞지 않는 아이템, UI에 넣기 어려운 긴 응답.',
            '추천 결과를 바로 믿지 않고, 화면에 들어갈 수 있는 구조와 사용자가 납득할 수 있는 설명인지 비교했다.',
          ],
        ...
        },
        {
          ...
          how: [
            'AI 제안은 초안, 대안, 검토용으로 사용하고 제품 방향은 직접 결정했다.',
            'PickFit의 핵심을 “예쁜 패션 앱”이 아니라 “결정 피로를 줄이는 AI 추천 경험”으로 정했다.',
            '화면을 크게 보여줄 것과 줄일 것을 구분했다. 인사이트와 독립 디자인 시스템은 키우고, 일반 UI 화면은 작게 묶었다.',
          ],
        ...
        items: [
          {
            ...
            caption: '인사이트 — 쇼핑 UX를 탐색형에서 위임형 추천 흐름으로 재정의',
            layout: 'wide',
          },
        ],
      },
        ...
        items: [
          {
            ...
            caption: '독립 디자인 시스템 — 컬러·타이포·컴포넌트를 처음부터 직접 설계',
            layout: 'wide',
          },
        ],
      },
    ...
    infoDock: {
      problem: [
        '패션 쇼핑은 상품, 리뷰, 코디, 핏, 예산을 동시에 비교해야 해서 선택 피로가 크다.',
        '사용자는 더 많은 리스트보다 “내 상황에 맞는 선택지”와 “왜 이 코디인지”를 먼저 필요로 한다.',
      ],
      insight: [
        '쇼핑 UX는 클릭해서 찾는 경험에서 조건을 맡기고 추천을 검토하는 경험으로 이동하고 있다고 보았다.',
        '그래서 PickFit은 상품 탐색 앱이 아니라 사용자의 결정을 줄여주는 AI 코디 추천 앱으로 기획했다.',
      ],
      role: [
      ...
      coreFlow: [
        '상황 선택으로 추천 맥락을 먼저 좁힌다.',
        '스타일, 예산, 색상, 핏 조건을 입력해 AI가 이해할 수 있는 추천 조건으로 구조화한다.',
        'AI 추천 결과에는 코디 이미지·설명뿐 아니라 추천 이유와 구성 아이템까지 함께 보여준다.',
        '마지막에는 전체 코디와 개별 아이템을 확인해 사용자가 구매 또는 저장으로 넘어갈 수 있게 한다.',
      ],
      output: [
      ...
      uxWritingPoint: [
        '“어떤 상황의 룩이 필요해요?”처럼 사용자가 조건을 쉽게 말할 수 있는 질문형 문구를 사용했다.',
        '추천 결과에는 단순 결과보다 예산, 상황 적합성, 추천 이유를 함께 보여줘 납득 가능성을 높였다.',
      ],
      learning: [
        'AI 추천은 결과가 똑똑해 보이는 것보다 사용자가 믿고 고를 수 있는 설명 구조가 중요했다.',
        '기획, 디자인, 구현, 데이터 구조를 혼자 연결하면서 AI 제품에서는 각 단계의 입력/출력 정의가 곧 UX라는 점을 배웠다.',
      ],
    },
  },
  likelion: {
    ...
    objectPack: '/images/design-assets/object-packs/likelion_objectpack.png',
    entranceLine:
    ...
    casePanels: [
      {
        ...
        body:
          '커리큘럼 원문은 “문제를 발견하고, 사용자를 이해하고, 구조와 로직과 상태를 설계하며, 실전 협업과 검증, 개선까지 경험하는 과정”을 목표로 둔다. 그래서 단순 강의 목록이 아니라 초보자가 다음 행동을 알 수 있는 학습 여정으로 구성했다.',
      },
      {
        eyebrow: '4-phase flow',
        title: '기획 기본기에서 회고와 문제 재정의까지',
        body:
          '초반부는 Why·Who·What·IA로 기획의 뼈대를 만들고, 중반부는 화면 로직·상태·예외 설계로 구체화한다. 후반부는 데이터·비즈니스·AI 활용으로 확장하고, 마지막은 해커톤 이후 회고와 문제 재정의로 닫는다.',
      },
      {
        eyebrow: 'operation system',
        title: '강의자료, 과제, 운영보드, 카드뉴스까지 함께 운영',
        body:
          'Figma 운영보드와 카드뉴스는 단순 홍보물이 아니라 교육 운영의 접점이었다. 모집 커뮤니케이션, 파트 안내, 세션 준비, 현장 멘토링까지 한 흐름으로 관리했다.',
      },
    ],
    writingRefs: [
      {
        ...
        application:
          '1주차 UX/UI 트랙 이해와 역기획 파트에서 “예쁜 화면”보다 구조와 의도를 먼저 읽는 기준으로 연결했다.',
      },
      {
        ...
        application:
          '2주차 Why·Who·페르소나 실습에서 추상적인 아이디어를 실제 사용자 문제로 바꾸는 흐름으로 반영했다.',
      },
      {
        ...
        application:
          '3주차 What·MVP·MoSCoW 수업에서 “이번에 만들 것/만들지 않을 것”을 정하는 판단 기준으로 사용했다.',
      },
    ],
    infoDock: {
      problem:
        "초보자는 PM/UX 개념을 들어도 실제 프로젝트 흐름으로 연결하지 못한다. 강의는 많지만 '그래서 다음에 무엇을 해야 하는지'를 알려주는 구조가 드물다.",
      insight:
        "교육은 개념을 많이 알려주는 것이 아니라 '다음 행동을 알 수 있게 만드는 구조'여야 한다. 그래서 커리큘럼 자체를 결정 가능한 흐름으로 설계했다.",
      role:
      ...
      output:
        '막연한 아이디어가 결정 가능한 기획으로 발전하도록 20주 과정, 과제, 세션 자료, 운영 보드를 설계하고 직접 강의했습니다.',
      verification:
      ...
      learning:
        '가르칠 수 있는 구조로 바꾸는 과정이 곧 개념을 소유하고 있다는 증명이며, 교육 설계 또한 하나의 제품 설계라는 것을 배웠습니다.',
    },
  },
}
